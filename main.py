...
def login():
    ...
    if form.validate_on_submit():
        ...
        # Checking the email that the user entered.
        if user:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            new_comment = Comment(author_id=current_user.id, post_id=post_id, text=form.comment_text.data)
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        # If id is not 1 or current_user.is_anonymous then return abort with 403 error
